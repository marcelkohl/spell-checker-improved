...
export default {
    ...
    enableProjectWatcher() {
        ...
        atom.project.onDidChangePaths((projectPath)=>{
            self.loadKnownWords(projectPath[0]);
        });
    },
...
//                console.log("stop changing disposed", edimissSpelledtor.id);
    ...
    // load known words
    loadKnownWords(projectPath) {
        let filePath = projectPath + '/known-words.json';
        let fs = require("fs");

        let data = fs.readFileSync(filePath); //file exists, get the contents
        let knownWords = JSON.parse(data); //turn to js object
