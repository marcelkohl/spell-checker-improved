...
export default {
    ...
    suggestionOnPlugins(misspelledWord) {
        return this.subscribedPlugins.reduce((suggestions, subscribedPlugin)=>{
        	return suggestions.concat(subscribedPlugin.onGetSuggestions(misspelledWord));
        }, []);
    },

    spellCheckOnPlugins(misspelledWords) {
        return misspelledWords.filter((word)=>{
            return !this.subscribedPlugins.some(
                (plugin)=>{
