    ...
    enableObservers() {
        ...
        atom.workspace.observeTextEditors((editor) => {
            ...
            let pCheckSpelling = (bufferText)=>new Promise((resolve)=>{
                //let misspelled = self.checkMisspelling(self.checkMisspelling(self.checkMisspelling(bufferText).join(' ')).join(' ')).join(' ');
                self.checkMisspelling(bufferText).then(resolve);
            });

/*            let checkSpelling = ()=> {
                let t0 = performance.now();
                let mispelledCheck = (bufferText)=>new Promise((resolve)=>{
                    self.checkMisspelling(bufferText).then(resolve);
                    //let misspelled = self.checkMisspelling(self.checkMisspelling(self.checkMisspelling(bufferText).join(' ')).join(' ')).join(' ');
                    //resolve(misspelled);
                });

                mispelledCheck(buffer.getText()).then((msp)=>console.log(msp.length));
...
//                spellCheckerView.setMarkers(missSpelled);
            ...
            let stopChangingSubscription = editor.onDidStopChanging(() => {
                console.log('stopchange pCheckSpelling begin');
                //setTimeout(
                let t0 = performance.now();
                    pCheckSpelling(buffer.getText())
                        .then((wordsToMark)=>{
                            //console.log('wordsToMark', wordsToMark)
                            spellCheckerView.setMarkers(wordsToMark);
                        })
                        //.then((msp)=>console.log(msp.length))
                //    , 100
                //);
                console.log('stopchange pCheckSpelling end');
                //checkSpelling();
                let t2 = performance.now();
                console.log("sp-chk all" + (t2 - t0).toFixed(2) );
            });
...
//            checkSpelling();
    ...
    //        return text.match(/((\w[\w']*(?:-\w+)*'[a-zA-Z])|([A-Z]*([a-z]){2,})|([A-Z]){2,})/g)
            ...
            this.removeKnownWordsFromWords(wordsInText, this.projectKnownWords)
                .then(
                    (wit)=>this.spellcheckerManager.isMisspelled(wit)
                        .then((mspldwords)=>{
                            resolve(mspldwords)
                        })
                    // (wit)=>resolve(
                    //     wit.filter(word => this.spellcheckerManager.isMisspelled(word))
                    // )
                );
    ...
    removeKnownWordsFromWords(sourceWords, knownWords) {
        return new Promise((resolve)=>{
            ...
            //console.log('sourceStr split', sourceStr.split(' '));
            resolve(sourceStr.split(' '));
        })
    },
