...
export default {
    ...
    _enableObservers() {
        ...
        self.disposables.add(
            atom.workspace.observeTextEditors((editor) => {
                ...
                self._onClickStatusBar(editor.id, ()=>{
                    if (spellCheckerView.lastMarkedWords.length > 0) {
                        let cursorPosition = editor.getCursorBufferPosition();
                        let nextRange;

                        for (var i = 0; i < spellCheckerView.lastMarkedWords.length; i++) {
                            let markedWord = spellCheckerView.lastMarkedWords[i];

                            if (
                                markedWord.range.start.row > cursorPosition.row ||
                                (markedWord.range.start.row === cursorPosition.row && markedWord.range.start.column > cursorPosition.column)
                            ) {
                                nextRange = markedWord.range;
                                break;
                            }
                        };

                        nextRange = nextRange || spellCheckerView.lastMarkedWords[0].range;

                        editor.setCursorBufferPosition(nextRange.start);
                        editor.setSelectedBufferRange(nextRange);
                    }
                });
    ...
    checkMisspelling(text) {
        return new Promise((resolve)=>{
            ...
            let uniqueWordsInText = (new WordsFromTextBuilder(text)).getUniqueWords();
