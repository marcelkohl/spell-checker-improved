...
export default {
    ...
    enableObservers() {
        ...
        atom.workspace.observeTextEditors((editor) => {
            ...
            let checkOnPlugins = (wordsToMark)=>wordsToMark.filter((word)=>{
                    return !self.subscribedPlugins.some(
                        (plugin)=>{
            ...
            let setMarkers = (wordsToMark)=>{
                wordsToMark = checkOnPlugins(wordsToMark);
            ...
            let stopChangingSubscription = editor.onDidStopChanging(() => {
                self.pCheckSpelling(buffer.getText()).then(setMarkers);
            });
