...
export default class SpellcheckerManager {
    ...
    loadDictionary (locale, path) {
        console.log('dictionary loaded', spellchecker);
  ...
  isMisspelled(wordsToCheck) {
      return new Promise((resolve)=>{
          let wordsStr = wordsToCheck.join(' ');

          spellchecker.checkSpellingAsync(wordsStr)
            .then((misspPos)=>{
                resolve(
                    misspPos.map((pos)=>{ return wordsStr.substring(pos.start, pos.end)})
                )
            });
          //spellchecker.isMisspelled(wordToCheck).then(resolve)
