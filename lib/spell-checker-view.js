...
export default class SpellCheckerView {
    ...
    setMarkers(misspelledWords) {
        ...
        if (misspelledWords && misspelledWords.length) {
            ...
            //TODO: scan range scluding folded ranges :\
    ...
    /*
    getFoldedInRange = (fl, ll)=>{
        foldeds = [];

        Object.keys(temp1.displayLayer.foldsMarkerLayer.markersById).forEach((foldId)=> foldeds.push(temp1.displayLayer.foldsMarkerLayer.markersById[foldId].getRange()));

        return foldeds.filter((folded)=>folded.start.row >= fl && folded.end.row < ll);
    }

    temp4.reduce((ac, va, id, ar)=>{
    	let res = ar.filter((fold)=>va.start.row > fold.start.row && va.end.row <= fold.end.row);
    	console.log('-->', res, va);
    	return res.length == 0 ? ac.concat(va) : ac;
    }, [])

    rangesToCheck = (fl, ll, folds)=>{
    	let sortedFolds = folds.sort((a, b)=>{
    		return a - b;
        });

    	let ranges = [];
    	sortedFolds.forEach((sFold)=>{
        	ranges.push({
    			start: {row: },
    			end: {}
    		});
