...
import WordsFromTextBuilder from './builder/words-from-text';
...
export default class SpellCheckerView {
    ...
    setMarkers(misspelledWords) {
        ...
        if (misspelledWords && misspelledWords.length) {
            ...
            let firstLine = editor.bufferRowForScreenRow(editor.getFirstVisibleScreenRow() || 0);
            let lastLine = editor.bufferRowForScreenRow(editor.getLastVisibleScreenRow() || 0);

            let textInRange = editor.getTextInRange([[firstLine, 0], [lastLine, 9999]]);
            let uniqueWordsInTextRange = (new WordsFromTextBuilder(textInRange)).getUniqueWords();
            let misspelledInRange = misspelledWords.filter((misspelledWord)=>uniqueWordsInTextRange.indexOf(misspelledWord) > -1);

            let markerLayer = this.markerLayer;
            let match1 = misspelledInRange.filter((w)=>w[0]==w[0].toUpperCase());
            let match1Str = match1.join('|') + '(?!=*[a-z])';
            let match2 = misspelledInRange.filter(w => !match1.includes(w));
        ...
        this._lastMarkedWords = markedWords;
        this.callbackOnMisspellMarked(this._lastMarkedWords, timersAt);
    }
